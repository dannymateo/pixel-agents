...
server/                              Lifecycle runtime + Fastify HTTP/WS server
  src/
    providers/hook/claude/           Reference HookProvider — only place that knows Claude specifics
      ...
      claudeTeamProvider.ts          TeamProvider: reads ~/.claude/teams/<name>/config.json (teamName validated at the edge by `isSafeTeamName`; invalid = no team, no disk read)
      ...
      claudeConversation.ts          Transcript record -> conversations (Agent/Task assign, SubagentHandback report, SendMessage)
    ...
    conversations.ts                 ConversationTracker: who talks to whom → `agentConversation` broadcasts
...
- **37 ServerMessage variants** (server → client): agent lifecycle, agent activity, sub-agent activity, team + context usage, assets, settings + workspace, diagnostics, living office (`agentPresence`, `livingOfficeSettings`), agent screen feed (`agentFeedSnapshot/Append/Denied`, point-to-point), conversations (`agentConversation`, `text` stripped for unprivileged connections).
...
- **Conversations** (spec §4b): `ConversationTracker` (`server/src/conversations.ts`) turns `HookProvider.parseConversations` results into `agentConversation { fromId, toId?, kind, text? }` — `assign` when the child materializes (after its `agentCreated`), `report` on the child's `SubagentHandback` (or, without one, the foreground spawn's `tool_result`; never both), `message` on `SendMessage` resolved by `spawnAgentKey` then `agentName` inside the sender's tree. Only live records speak: an agent added to the store replays silently until its first record stamped after it arrived (records need a `timestamp`), and a re-read from offset 0 calls `beginReplay`. The webview's `ConversationDirector` stages one scene per speaker (FIFO): walk next to the listener, typewriter bubble, walk back; ✉ only when the recipient is unresolved.
