...
99 tests total. Generated by `scripts/generate-e2e-inventory.mjs`. Re-run after adding or removing tests.
...
- `e2e/claude/hooks-off/livingOffice.spec.ts:100` — living office: a team gets a named module, completed stays, killed walks out and frees it (Hooks OFF / living office)
- `e2e/claude/hooks-off/livingOffice.spec.ts:213` — living office: a team that finishes before reaching its desks ends seated in its module (Hooks OFF / living office)
- `e2e/claude/hooks-off/spawnTree.spec.ts:47` — spawn tree three levels deep renders every node and cascades on the root spawn result (Hooks OFF / spawn tree)
- `e2e/claude/hooks-off/spawnTree.spec.ts:165` — spawn tree background spawn without SubagentStart materializes from a late sidecar and leaves when stopped (Hooks OFF / spawn tree)
- `e2e/claude/hooks-off/spawnTree.spec.ts:242` — spawn tree grandchild written before its parent appears once the parent exists (Hooks OFF / spawn tree)
- `e2e/claude/hooks-on/spawnTree.spec.ts:51` — spawn tree hooks with agent_id animate only their own node, never the lead (Hooks ON / spawn tree)
...
- `e2e/claude/hooks-on/teams.spec.ts:424` — external session lead with inline teammate routes tools to teammate (Hooks ON / teams)
- `e2e/claude/hooks-on/teams.spec.ts:496` — external session lead with tmux teammate routes tools to teammate (Hooks ON / teams)
