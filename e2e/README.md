...
- `e2e/claude/hooks-off/spawnTree.spec.ts:165` — spawn tree background spawn without SubagentStart materializes from a late sidecar and leaves on completion (Hooks OFF / spawn tree)
- `e2e/claude/hooks-off/spawnTree.spec.ts:241` — spawn tree grandchild written before its parent appears once the parent exists (Hooks OFF / spawn tree)
